...
class FirstSearch():
    def __init__(self, number_of_vertices, list_of_adjacent_edges_of_graph):
        ...
        self.used = [False for _ in range(self.number_of_vertices)]
...
class TopologicalSort(FirstSearch):
    ...
    def topological_sort_from_start_vertex(self, v):
        self.used[v] = True
        for to in self.list_of_adjacent_edges_of_graph[v]:
            if not self.used[to]:
                self.topological_sort_from_start_vertex(to)
        self.order_topological_sort.insert(0, v)

    def run(self):
        for v in range(self.number_of_vertices):
            if not self.used[v]:
                self.topological_sort_from_start_vertex(v)
...
class StronglyConnectedComponent(TopologicalSort):
    ...
    def build_transposed_graph(self):
